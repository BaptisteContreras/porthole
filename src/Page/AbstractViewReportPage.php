...
abstract class AbstractViewReportPage implements PageInterface
{
    public function __construct(
        protected readonly CsvReader $reader,
        protected readonly HarborContext $context,
        protected readonly GenerateReportHandler $handler,
    ) {
    }

    abstract protected function buildView(): UserReportView|ImageReportView;

    final public function mount(Navigator $navigator): ContainerWidget
    {
        ...
        $this->activateTab($firstKey, $filterContainer, $dataContainer, $hint, $shaDetail, $navigator, $buildDataList, $hintText);
        ...
        $tabWidget->onSelect(function (SelectEvent $event) use (
            ...
            $this->activateTab($item['value'], $filterContainer, $dataContainer, $hint, $shaDetail, $navigator, $buildDataList, $hintText);
            $navigator->requestPageRender();
        });
    ...
    protected function activateTab(
        ...
        $shaDetail->setText('');
        $dataContainer->clear();
        $rows = $this->buildView()->rows($tabKey);
