...
final class ViewImageReportPage extends AbstractViewReportPage
{
    ...
    protected function activateTab(
        ...
        $imageFilterInput->onChange(function (ChangeEvent $event) use ($dataList, $hint, $navigator, $hintText, $view, &$imageFilter, &$tagFilter): void {
            $imageFilter = $event->getValue();
            $items = $this->filterRows($imageFilter, $tagFilter, $view);
        ...
        $tagFilterInput->onChange(function (ChangeEvent $event) use ($dataList, $hint, $navigator, $hintText, $view, &$imageFilter, &$tagFilter): void {
     ...
     */
    private function filterRows(string $imagePrefix, string $tagPrefix, ImageReportView $view): array
    {
        $imagePrefix = strtolower($imagePrefix);
        $tagPrefix = strtolower($tagPrefix);

        if ('' === $imagePrefix && '' === $tagPrefix) {
        ...
        $filtered = array_values(array_filter(
            $this->report->rows,
            static fn (ImageReportRow $r) => ('' === $imagePrefix || str_starts_with(strtolower($r->image), $imagePrefix))
